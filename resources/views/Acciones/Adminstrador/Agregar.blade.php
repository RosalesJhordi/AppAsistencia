...
@section('contenido')
<div class="w-full px-10 mt-10 flex justify-center conte-forms">
    <form action="{{route('Agregar')}}" method="POST" class="form py-5 shadow-md flex-col border w-1/3 justify-center items-center p-2 flex gap-10 flex-wrap">
        @csrf
    ...
    <div class="w-1/3 border shadow-md form">
        <form action="{{route('Horario.post')}}" method="POST" class="w-full h-full  py-5 flex-col border items-center p-2 flex gap-10 flex-wrap">
            ...
            <button type="submit" class="w-1/2 bg-blue-600 text-white p-2 mt-10 rounded-md">Establecer</button>
        </form>
    </div>
</div>
